...
type Props = { kind: PageKind; eyebrow: string; title: string; lead: string; description: string; image: string; status: string; focus: string; steps: Array<[string, string, "done" | "now" | "next"]>; next: string; nextHref: string };

export default function MediaPage({ kind, eyebrow, title, lead, description, image, status, focus, steps, next, nextHref }: Props) {
  return <div className="page-shell" id="top"><SiteHeader current={kind} /><main>
    ...
    <section className="current-section" id="current"><div><p className="eyebrow eyebrow-dark">THE WORK NOW</p><h2>這一站，<br /><em>正在發生。</em></h2><p>每一個階段都有自己的節奏。我們把作品如何往下一站前進，清楚留在這裡。</p></div><article className="status-card"><p><CircleDot size={14} /> {status}</p><strong>{focus}</strong><span>這次累積的內容，會成為下一種媒介可以承接的故事素材。</span></article></section>
