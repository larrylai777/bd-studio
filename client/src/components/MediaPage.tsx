...
type Props = { kind: PageKind; eyebrow: string; title: string; lead: string; description: string; image: string; status: string; focus: string; currentCover?: string; listenHref?: string; listenLabel?: string; steps: Array<[string, string, "done" | "now" | "next"]>; next: string; nextHref: string };

export default function MediaPage({ kind, eyebrow, title, lead, description, image, status, focus, currentCover, listenHref, listenLabel, steps, next, nextHref }: Props) {
