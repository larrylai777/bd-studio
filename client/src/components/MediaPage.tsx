...
type Props = { kind: PageKind; eyebrow: string; title: string; lead: string; description: string; image: string; status: string; focus: string; currentCover?: string; currentCoverHref?: string; listenHref?: string; listenLabel?: string; steps: Array<[string, string, "done" | "now" | "next"]>; next: string; nextHref: string };

export default function MediaPage({ kind, eyebrow, title, lead, description, image, status, focus, currentCover, currentCoverHref, listenHref, listenLabel, steps, next, nextHref }: Props) {
  return <div className="page-shell" id="top"><SiteHeader current={kind} /><main>
    ...
    <section className="current-section" id="current"><div><p className="eyebrow eyebrow-dark">THE WORK NOW</p><h2>這一站，<br /><em>正在發生。</em></h2><p>每一個階段都有自己的節奏。我們把作品如何往下一站前進，清楚留在這裡。</p></div>{currentCover ? currentCoverHref ? <a className="status-card status-card-cover" href={currentCoverHref} target="_blank" rel="noreferrer" aria-label="閱讀《第九次出生》"><img src={currentCover} alt="《第九次出生》封面" /></a> : <article className="status-card status-card-cover"><img src={currentCover} alt="《第九次出生》封面" /></article> : <article className="status-card"><p><CircleDot size={14} /> {status}</p><strong>{focus}</strong><span>這次累積的內容，會成為下一種媒介可以承接的故事素材。</span>{listenHref && <a className="listen-link" href={listenHref} target="_blank" rel="noreferrer">{listenLabel ?? "前往收聽"}<ArrowRight size={16} /></a>}</article>}</section>
