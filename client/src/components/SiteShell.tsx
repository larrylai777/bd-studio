...
export function SiteHeader({ current }: { current: Current }) {
  ...
  const notice = (label: string) => toast(`${label} 即將開放`, { description: "下一道 BÐ-Studio 訊號會在官方網站公布。" });
  ...
  return <header className="entertainment-header">
    ...
    <nav className="desktop-primary" aria-label="桌面主要導覽">{links.map(([label, href]) => <a className={current === (label === "原作" ? "original" : label === "播客" ? "podcast" : "merch") ? "active" : ""} href={href} key={label}>{label}</a>)}</nav>
    {menuOpen && <nav className="header-panel menu-panel" aria-label="行動版主要導覽"><a href={assetBase} onClick={() => setMenuOpen(false)}><span>00</span>首頁<ArrowRight size={17} /></a>{links.map(([label, href], index) => <a href={href} onClick={() => setMenuOpen(false)} key={label}><span>{String(index + 1).padStart(2, "0")}</span>{label}<ArrowRight size={17} /></a>)}<a href={`${assetBase}support/`} onClick={() => setMenuOpen(false)}><span>04</span>支持連載<ArrowRight size={17} /></a></nav>}
    {searchOpen && <div className="header-panel search-panel"><label htmlFor="site-search">SEARCH THE BÐ UNIVERSE</label><div><input id="site-search" autoFocus placeholder="搜尋作品、角色或世界觀" /><button onClick={() => notice("搜尋")}>搜尋</button></div></div>}
  </header>;
}
...
export function SiteFooter() {
  ...
  return <footer className="site-footer-expanded">
    <section className="footer-main">
      ...
      <nav className="footer-links" aria-label="頁尾導覽"><a href={`${assetBase}support/`}>支持連載</a><a href="mailto:bbdaddy924@gmail.com">合作洽談</a></nav>
